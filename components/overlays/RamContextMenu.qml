...
PopupWindow {
            ...
            Column {
                ...
                Column {
                    ...
                    ConfigToggleItem {
                        width: parent.width
                        label: "Label"
                        value: getConfigValue("showLabel", false) ? "\"RAM\"" : "Hidden"
                    ...
                    ConfigToggleItem {
                        width: parent.width
                        label: "Show Total"
                        value: getConfigValue("showTotal", true) ? (currentUsage.toFixed(1) + "/" + totalAvailable.toFixed(1) + " GB") : "Hidden"
                        isActive: getConfigValue("showTotal", true)
    ...
    function getConfigValue(key, defaultValue) {
        ...
        return configService.getValue("ram." + key, defaultValue)
    }
    
    function toggleConfig(key) {
        if (!configService) return
        
        const configKey = "ram." + key
    ...
    function generateDisplayPreview() {
        ...
        if (getConfigValue("showLabel", false)) parts.push("RAM")
        
        let ramParts = []
        if (getConfigValue("showPercentage", true)) {
            ramParts.push(usagePercent.toFixed(getConfigValue("precision", 0)) + "%")
        }
        if (getConfigValue("showTotal", true)) {
    ...
    // Update live data
    function updateData(usage, total, percent, frequency) {
    ...
    function navigateToParent() {
        ...
        if (parentComponent && typeof parentComponent.menu === 'function') {
            console.log(`[RamContextMenu] Navigating to parent: ${parentComponentId}`)
            ...
            console.warn(`[RamContextMenu] Parent component ${parentComponentId} not found or doesn't support menu()`)
        }
    }
    ...
    function navigateToChild(childId) {
        ...
        if (childComponent && typeof childComponent.menu === 'function') {
            console.log(`[RamContextMenu] Navigating to child: ${childId}`)
            ...
            console.warn(`[RamContextMenu] Child component ${childId} not found or doesn't support menu()`)
        }
    }
}